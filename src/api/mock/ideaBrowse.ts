// src/api/mock/ideaBrowse.ts

import type { CategoryConfig, IdeaComment, IdeaItem } from '@/api/types/ideaBrowse'
import { DEMO_USER_PROFILE } from './auth'

/** 현재 로그인된 사용자 (auth.ts의 DEMO_USER_PROFILE을 단일 출처로 사용) */
export const MY_AUTHOR = DEMO_USER_PROFILE.name

// ── 좋아요 mock 데이터 (아이디어별 20명) ──────────────────────────────────────
const LIKES: Record<number, string[]> = {
  1: [MY_AUTHOR, '김팀장', '박과장', '이사원', '최대리', '정차장', '한주임', '송과장', '윤팀장', '강사원',
      '임대리', '조부장', '류과장', '엄대리', '신주임', '권대리', '오과장', '배사원', '남팀장', '구대리'],
  2: ['이사원', '최대리', '한주임', '정차장', '송과장', '윤팀장', '강사원', '임대리', '김AI', '박안전',
      '조부장', '류과장', '엄대리', MY_AUTHOR, '신주임', '권대리', '오과장', '배사원', '남팀장', '구대리'],
  3: ['김팀장', '이사원', '박과장', '정차장', '최대리', '한주임', '송과장', '윤팀장', '강사원', '임대리',
      '조부장', MY_AUTHOR, '류과장', '엄대리', '신주임', '권대리', '오과장', '배사원', '남팀장', '구대리'],
  4: ['정차장', '송과장', '윤팀장', '강사원', '임대리', '김팀장', MY_AUTHOR, '박과장', '최대리', '한주임',
      '이사원', '조부장', '류과장', '엄대리', '신주임', '권대리', '오과장', '배사원', '남팀장', '구대리'],
  5: ['김팀장', '이사원', '최대리', '한주임', '박안전', '조부장', '송과장', '윤팀장', MY_AUTHOR, '강사원',
      '임대리', '류과장', '엄대리', '신주임', '권대리', '오과장', '배사원', '남팀장', '구대리', '정차장'],
  6: ['이사원', '최대리', '한주임', MY_AUTHOR, '강사원', '김팀장', '박과장', '정차장', '송과장', '윤팀장',
      '임대리', '조부장', '류과장', '엄대리', '신주임', '권대리', '오과장', '배사원', '남팀장', '구대리'],
  14: ['윤팀장', '강사원', '임대리', '김팀장', '박과장', '이사원', '최대리', '한주임', '정차장', '송과장',
       MY_AUTHOR, '조부장', '류과장', '엄대리', '신주임', '권대리', '오과장', '배사원', '남팀장', '구대리'],
}

// ── 댓글 mock 데이터 (아이디어별 10개) ────────────────────────────────────────
const COMMENTS: Record<number, IdeaComment[]> = {
  1: [
    { id: 1,  author: '김팀장',  content: 'LED 교체는 실제로 효과가 큽니다. 타 부서에서도 도입 검토 중이에요.', createdAt: '2026-02-21' },
    { id: 2,  author: '박과장',  content: '투자 회수 기간 분석 자료도 함께 첨부하면 심사에 유리할 것 같습니다.', createdAt: '2026-02-22' },
    { id: 3,  author: MY_AUTHOR, content: '네, 추가로 에너지 절감 사례 데이터도 준비하겠습니다!', createdAt: '2026-02-22' },
    { id: 4,  author: '이사원',  content: '생산라인 특성상 방열 문제도 검토해보셨나요?', createdAt: '2026-02-23' },
    { id: 5,  author: '정차장',  content: '이미 유사 사례에서 전기료 40% 절감 효과를 확인했습니다.', createdAt: '2026-02-23' },
    { id: 6,  author: '한주임',  content: '교체 시 폐형광등 처리 방법도 함께 계획하면 좋겠어요.', createdAt: '2026-02-24' },
    { id: 7,  author: '최대리',  content: '예산 승인 절차가 빠를수록 올해 안에 도입 가능할 것 같습니다.', createdAt: '2026-02-24' },
    { id: 8,  author: '송과장',  content: '안전 인증 제품으로 선정하면 보험료 절감도 기대할 수 있어요.', createdAt: '2026-02-25' },
    { id: 9,  author: '윤팀장',  content: '우선 파일럿 구역부터 테스트해보는 건 어떨까요?', createdAt: '2026-02-25' },
    { id: 10, author: '강사원',  content: '적극 찬성합니다. 현장에서도 밝기 개선 요청이 많았어요!', createdAt: '2026-02-26' },
  ],
  2: [
    { id: 11, author: '이사원',  content: '카페 메뉴 다양화 정말 필요합니다. 특히 비건 옵션이 있으면 좋겠어요.', createdAt: '2026-02-18' },
    { id: 12, author: '최대리',  content: '간식 종류도 늘려주면 좋겠습니다! 공감합니다.', createdAt: '2026-02-19' },
    { id: 13, author: '한주임',  content: '음료 구독 서비스와 연계하면 비용을 낮출 수 있을 것 같아요.', createdAt: '2026-02-20' },
    { id: 14, author: '정차장',  content: '점심 후 커피 한 잔이 간절한데 메뉴가 너무 단조로워요. 찬성!', createdAt: '2026-02-20' },
    { id: 15, author: '송과장',  content: '직원 복지 차원에서 일부 보조금 지원도 검토해주세요.', createdAt: '2026-02-21' },
    { id: 16, author: MY_AUTHOR, content: '건강 음료 옵션 추가도 함께 제안하면 어떨까요?', createdAt: '2026-02-21' },
    { id: 17, author: '윤팀장',  content: '타사 카페 운영 사례를 참고하면 좋을 것 같습니다.', createdAt: '2026-02-22' },
    { id: 18, author: '강사원',  content: '계절 한정 메뉴도 추가되면 더 좋겠어요!', createdAt: '2026-02-22' },
    { id: 19, author: '임대리',  content: '가격도 합리적으로 조정해주세요. 외부 카페보다 비싸면 안 쓸 것 같아요.', createdAt: '2026-02-23' },
    { id: 20, author: '조부장',  content: '임직원 할인 제도도 함께 도입하면 이용률이 올라갈 것 같습니다.', createdAt: '2026-02-23' },
  ],
  4: [
    { id: 21, author: '정차장',  content: '화상회의 품질 개선은 재택 효율과 직결됩니다. 적극 찬성합니다.', createdAt: '2026-02-14' },
    { id: 22, author: '송과장',  content: '기존 시스템과의 호환성 검토도 필요할 것 같습니다.', createdAt: '2026-02-15' },
    { id: 23, author: MY_AUTHOR, content: 'Teams Premium 가격 대비 효과 분석 자료를 공유해 드릴게요.', createdAt: '2026-02-15' },
    { id: 24, author: '한주임',  content: 'AI 요약 기능이 정말 매력적이에요. 회의록 작성 시간이 많이 줄겠네요.', createdAt: '2026-02-16' },
    { id: 25, author: '이사원',  content: '재택 인원이 많은 저희 팀에서 꼭 필요한 기능입니다.', createdAt: '2026-02-16' },
    { id: 26, author: '윤팀장',  content: '라이선스 비용은 어떻게 책정되나요? 인당 요금인가요?', createdAt: '2026-02-17' },
    { id: 27, author: '강사원',  content: '보안 설정도 기존보다 강화된다고 하니 더 좋을 것 같아요.', createdAt: '2026-02-17' },
    { id: 28, author: '임대리',  content: '파일럿 부서를 먼저 선정해서 테스트해보면 좋겠습니다.', createdAt: '2026-02-18' },
    { id: 29, author: '조부장',  content: 'IT팀과 사전 협의가 필요할 것 같습니다. 인프라 확인 요청드립니다.', createdAt: '2026-02-18' },
    { id: 30, author: '김팀장',  content: '좋은 제안입니다. 심사 때 우선 순위로 검토하겠습니다.', createdAt: '2026-02-19' },
  ],
  14: [
    { id: 31, author: '윤팀장',  content: '유연근무제 도입은 인재 유치에도 큰 도움이 됩니다.', createdAt: '2026-02-10' },
    { id: 32, author: '강사원',  content: '파일럿 프로그램으로 먼저 시작해보면 어떨까요?', createdAt: '2026-02-11' },
    { id: 33, author: '임대리',  content: '성과 측정 방식도 같이 제안해주시면 더 설득력 있을 것 같아요.', createdAt: '2026-02-12' },
    { id: 34, author: MY_AUTHOR, content: '코어 타임 설정 덕분에 협업에도 지장이 없을 것 같아요.', createdAt: '2026-02-12' },
    { id: 35, author: '조부장',  content: '타 기업 사례를 보면 생산성이 오히려 높아졌다는 데이터가 있어요.', createdAt: '2026-02-13' },
    { id: 36, author: '김팀장',  content: '근태 관리 시스템 업그레이드도 함께 진행되면 좋겠습니다.', createdAt: '2026-02-13' },
    { id: 37, author: '박과장',  content: '가족 돌봄이 필요한 직원들에게 특히 유용한 제도가 될 것 같습니다.', createdAt: '2026-02-14' },
    { id: 38, author: '최대리',  content: '부서별로 운영 방침이 다를 수 있으니 가이드라인 표준화가 필요합니다.', createdAt: '2026-02-14' },
    { id: 39, author: '한주임',  content: '입사 6개월 이상 직원부터 적용하는 단계적 도입을 제안합니다.', createdAt: '2026-02-15' },
    { id: 40, author: '정차장',  content: '이 제안이 통과되면 이직률 감소에도 효과가 있을 거라 생각합니다.', createdAt: '2026-02-15' },
  ],
}

export const CATEGORY_CONFIG: CategoryConfig[] = [
  { id: '절감', label: '절감', emoji: '💰', color: '#f59e0b', bg: 'rgba(245,158,11,0.1)', border: 'rgba(245,158,11,0.35)' },
  { id: '혁신', label: '혁신', emoji: '🚀', color: '#6366f1', bg: 'rgba(99,102,241,0.1)', border: 'rgba(99,102,241,0.35)' },
  { id: '안전', label: '안전', emoji: '🛡️', color: '#10b981', bg: 'rgba(16,185,129,0.1)', border: 'rgba(16,185,129,0.35)' },
  { id: '복지', label: '복지', emoji: '❤️', color: '#ec4899', bg: 'rgba(236,72,153,0.1)', border: 'rgba(236,72,153,0.35)' },
  { id: '품질', label: '품질', emoji: '⭐', color: '#eab308', bg: 'rgba(234,179,8,0.1)', border: 'rgba(234,179,8,0.35)' },
  { id: '환경', label: '환경', emoji: '🌿', color: '#14b8a6', bg: 'rgba(20,184,166,0.1)', border: 'rgba(20,184,166,0.35)' },
  { id: '기타', label: '기타', emoji: '📌', color: '#94a3b8', bg: 'rgba(148,163,184,0.1)', border: 'rgba(148,163,184,0.35)' },
]

export const BIZ_AREAS = ['본사', '강남지사', '부산지사']

export const DEPT_BY_BIZ_AREA: Record<string, string[]> = {
  '본사':    ['개발팀', '기획팀', 'UX팀', '인사팀', '재무팀', '총무팀'],
  '강남지사': ['생산1부', '생산2부', '품질팀'],
  '부산지사': ['영업팀', '마케팅팀', '고객서비스팀'],
}

export const MY_IDEA_TITLES = [
  '사내 점심시간 30분 연장',
  '주 1회 재택근무 의무화',
  '휴가 사용 시 자동 승인 제도',
  '사내 간식 바 확대',
  '코드 리뷰 시간 단축 가이드라인',
]

export const MY_IDEA_KEYWORDS: string[][] = [
  ['점심', '식사', '점심시간', '배달'],
  ['재택', '재택근무', '원격근무', '유연근무'],
  ['휴가', '연차', '자동승인', '승인'],
  ['간식', '카페', '음료', '식음'],
  ['코드', '리뷰', '개발', '가이드라인'],
]

export const IDEAS: IdeaItem[] = [
  {
    id: 1,
    title: '생산라인 LED 조명 교체로 전기요금 절감',
    categoryId: 1, category: '절감',
    problem: '현재 형광등 사용으로 전기요금이 연간 2,400만원 이상 발생하고 있으며, 노후 조명으로 작업 환경이 불량합니다.',
    solution: 'LED 교체 시 연간 1,200만원 절감 예상. 3년 이내 투자 회수 가능. 작업 조도 개선으로 생산성 향상 기대.',
    author: MY_AUTHOR,
    bizArea: '강남지사',
    department: '생산1부',
    status: '심사대기',
    submittedAt: '2026-02-20',
    security: 'public',
    likes: 20,
    comments: 10,
    views: 120,
    commentsData: COMMENTS[1],
    likesData: LIKES[1],
  },
  {
    id: 2,
    title: '사내 카페 메뉴 다양화 및 간식 음료 확대',
    categoryId: 4, category: '복지',
    problem: '카페 메뉴가 단조로워 직원 만족도 낮음. 음료와 간식 종류가 부족하여 직원들이 외부 구매를 선호하고 있습니다.',
    solution: '계절 메뉴 + 건강 식음료·간식 확대. 월별 신메뉴 도입으로 직원 만족도 향상 예상.',
    author: '최복지',
    bizArea: '본사',
    department: '총무팀',
    status: '심사대기',
    submittedAt: '2026-02-22',
    security: 'public',
    likes: 20,
    comments: 10,
    views: 450,
    commentsData: COMMENTS[2],
    likesData: LIKES[2],
  },
  {
    id: 3,
    title: '사무실 공기청정기 추가 설치',
    categoryId: 3, category: '안전',
    problem: '미세먼지 농도 높아 직원 건강 우려. 최근 측정 결과 WHO 권고치의 1.8배 초과.',
    solution: '고성능 공기청정기 10대 설치. 연간 건강보험 비용 절감 및 결근 감소 기대.',
    author: '박안전',
    bizArea: '강남지사',
    department: '품질팀',
    status: '심사대기',
    submittedAt: '2026-02-25',
    security: 'public',
    likes: 20,
    comments: 10,
    views: 310,
    likesData: LIKES[3],
  },
  {
    id: 4,
    title: '재택근무 원격근무 화상회의 시스템 업그레이드',
    categoryId: 2, category: '혁신',
    problem: '현재 시스템 화질·지연 문제로 재택근무 및 원격근무 회의 효율 저하. 원격 협업 생산성 30% 하락 추정.',
    solution: 'Microsoft Teams Premium 도입. AI 기반 회의 요약 및 재택근무 환경 개선.',
    author: MY_AUTHOR,
    bizArea: '본사',
    department: '개발팀',
    status: '승인',
    submittedAt: '2026-02-10',
    security: 'public',
    likes: 20,
    comments: 10,
    views: 380,
    ideaScore: 82,
    mileageScore: 1500,
    commentsData: COMMENTS[4],
    likesData: LIKES[4],
  },
  {
    id: 5,
    title: 'AI 기반 고객 상담 챗봇 도입',
    categoryId: 2, category: '혁신',
    problem: '고객 문의 응대 지연 및 인력 부족으로 고객 불만이 증가하고 있습니다.',
    solution: 'ChatGPT 기반 챗봇 + 사내 지식베이스 연동으로 1차 응대 자동화.',
    author: '김AI',
    bizArea: '부산지사',
    department: '고객서비스팀',
    status: '승인',
    submittedAt: '2026-03-15',
    security: 'public',
    likes: 20,
    comments: 10,
    views: 290,
    ideaScore: 78,
    mileageScore: 1200,
    likesData: LIKES[5],
  },
  {
    id: 6,
    title: '사내 도서관 디지털화 프로젝트',
    categoryId: 2, category: '혁신',
    problem: '종이 책 관리 비용 과다 및 공간 부족. 직원 독서율도 저조한 상황.',
    solution: '전자책 플랫폼 도입 + RFID 대출 시스템으로 편의성 향상 및 공간 절약.',
    author: '정도서',
    bizArea: '본사',
    department: '인사팀',
    status: '심사대기',
    submittedAt: '2026-03-05',
    security: 'private',
    likes: 20,
    comments: 10,
    views: 200,
    likesData: LIKES[6],
  },
  {
    id: 7,
    title: '사옥 옥상 태양광 패널 설치',
    categoryId: 6, category: '환경',
    problem: '전기요금 지속 상승 및 ESG 목표 미달성. 탄소중립 목표 달성 필요.',
    solution: '옥상 전체에 태양광 패널 설치 → 연간 800kWh 발전, 전기요금 25% 절감.',
    author: '한환경',
    bizArea: '강남지사',
    department: '품질팀',
    status: '심사대기',
    submittedAt: '2026-03-22',
    security: 'public',
    likes: 54,
    comments: 9,
    views: 180,
  },
  {
    id: 8,
    title: '점심 배달 사내 식사 서비스 제휴',
    categoryId: 4, category: '복지',
    problem: '점심시간이 부족하여 직원들이 빠른 식사를 위해 불균형 식사를 하는 경우가 많음. 배달 비용도 개인이 부담.',
    solution: '인근 식당과 제휴하여 사내 배달 서비스 제공. 그룹 주문 할인 혜택 포함하여 점심 식사 환경 개선.',
    author: '이복지',
    bizArea: '본사',
    department: '총무팀',
    status: '심사대기',
    submittedAt: '2026-02-28',
    security: 'public',
    likes: 89,
    comments: 19,
    views: 340,
  },
  {
    id: 9,
    title: '안전 보호구 교체 주기 단축',
    categoryId: 3, category: '안전',
    problem: '노후 안전 보호구 사용으로 작업 중 사고 위험 증가. 최근 3건의 경미한 부상 발생.',
    solution: '보호구 교체 주기를 1년에서 6개월로 단축. 작업자 안전 사고 50% 감소 목표.',
    author: '박안전',
    bizArea: '강남지사',
    department: '생산2부',
    status: '반려',
    submittedAt: '2026-01-15',
    security: 'private',
    likes: 23,
    comments: 7,
    views: 95,
  },
  {
    id: 10,
    title: '설비 예방보전 스케줄 최적화',
    categoryId: 1, category: '절감',
    problem: '설비 고장으로 인한 비계획 정지가 월 평균 12시간 발생. 생산 손실 연간 5억원 추정.',
    solution: 'IoT 센서 기반 예지 보전 시스템 도입으로 고장 사전 감지. 유지보수 비용 30% 절감.',
    author: '이생산',
    bizArea: '강남지사',
    department: '생산1부',
    status: '심사대기',
    submittedAt: '2026-02-05',
    security: 'public',
    likes: 41,
    comments: 12,
    views: 155,
  },
  {
    id: 11,
    title: '신규 고객 온보딩 가이드 개선',
    categoryId: 5, category: '품질',
    problem: '신규 고객 이탈률 높음. 온보딩 첫 2주 내 이탈률이 35%에 달함.',
    solution: '인터랙티브 튜토리얼 + 1:1 안내 콜 도입으로 온보딩 완료율 80% 달성 목표.',
    author: '박고객',
    bizArea: '부산지사',
    department: '고객서비스팀',
    status: '승인',
    submittedAt: '2026-01-20',
    security: 'public',
    likes: 38,
    comments: 14,
    views: 145,
    ideaScore: 75,
    mileageScore: 1000,
  },
  {
    id: 12,
    title: '영업팀 CRM 시스템 도입',
    categoryId: 2, category: '혁신',
    problem: '고객 정보가 엑셀에 분산 관리되어 영업 기회 누락 및 중복 접촉 빈번.',
    solution: 'Salesforce 도입으로 고객 통합 관리. 영업 성과 20% 향상 예상.',
    author: '최영업',
    bizArea: '부산지사',
    department: '영업팀',
    status: '실행중',
    submittedAt: '2025-11-10',
    security: 'public',
    likes: 55,
    comments: 16,
    views: 210,
    ideaScore: 88,
    mileageScore: 3000,
  },
  {
    id: 13,
    title: '탄소 배출량 실시간 모니터링 대시보드',
    categoryId: 6, category: '환경',
    problem: '탄소 배출량 파악이 분기 단위로만 이루어져 적시 대응이 불가능.',
    solution: '실시간 에너지 모니터링 대시보드 구축으로 탄소 배출 이상 즉시 감지 및 대응.',
    author: '한환경',
    bizArea: '강남지사',
    department: '품질팀',
    status: '심사대기',
    submittedAt: '2026-03-01',
    security: 'public',
    likes: 29,
    comments: 6,
    views: 88,
  },
  {
    id: 14,
    title: '유연근무제 전면 도입으로 재택근무 활성화',
    categoryId: 4, category: '복지',
    problem: '고정된 출퇴근 시간으로 직원 스트레스 증가. 재택근무 및 원격근무 수요가 높지만 제도 미비.',
    solution: '코어 타임(10-16시) 유지하는 유연근무제 + 재택근무 월 8일 허용. 워크-라이프 밸런스 향상.',
    author: '이인사',
    bizArea: '본사',
    department: '인사팀',
    status: '심사대기',
    submittedAt: '2026-02-15',
    security: 'private',
    likes: 20,
    comments: 10,
    views: 490,
    commentsData: COMMENTS[14],
    likesData: LIKES[14],
  },
  {
    id: 15,
    title: '사내 기술 세미나 정기화',
    categoryId: 2, category: '혁신',
    problem: '부서 간 기술 공유가 없어 중복 개발 및 기술 편차 발생. 사내 지식 사일로 현상.',
    solution: '격주 기술 세미나 운영. 발표자 마일리지 보상, 세미나 영상 사내 공유.',
    author: '박개발',
    bizArea: '본사',
    department: '개발팀',
    status: '완료',
    submittedAt: '2025-09-01',
    security: 'public',
    likes: 72,
    comments: 20,
    views: 260,
    ideaScore: 90,
    mileageScore: 5000,
  },
  {
    id: 16,
    title: '불량률 감소를 위한 공정 개선',
    categoryId: 5, category: '품질',
    problem: '현재 불량률 2.3%로 업계 평균 1.5% 대비 높음. 재작업 비용 연간 3억원 발생.',
    solution: '공정별 체크포인트 강화 + SPC 통계적 공정 관리 시스템 도입으로 불량률 1.0% 달성.',
    author: '김품질',
    bizArea: '강남지사',
    department: '생산2부',
    status: '심사대기',
    submittedAt: '2026-02-18',
    security: 'public',
    likes: 33,
    comments: 9,
    views: 112,
  },
  {
    id: 17,
    title: '직원 건강검진 항목 확대',
    categoryId: 4, category: '복지',
    problem: '기본 건강검진 항목 미흡으로 직원들의 조기 질병 발견이 어려움.',
    solution: '종합건강검진 비용 지원 확대. 암 조기 검진 항목 추가로 직원 건강 보호.',
    author: '박인사',
    bizArea: '본사',
    department: '인사팀',
    status: '승인',
    submittedAt: '2026-01-08',
    security: 'public',
    likes: 96,
    comments: 24,
    views: 380,
    ideaScore: 85,
    mileageScore: 1800,
  },
  {
    id: 18,
    title: '임직원 통근 교통비 지원 확대',
    categoryId: 4, category: '복지',
    problem: '원거리 통근 직원의 교통비 부담 증가로 이직률 상승. 특히 신입사원 이탈이 심각.',
    solution: '월 최대 20만원 교통비 지원 상향. 대중교통 이용 직원 우대 혜택 부여.',
    author: '최인사',
    bizArea: '본사',
    department: '인사팀',
    status: '심사대기',
    submittedAt: '2026-03-08',
    security: 'public',
    likes: 78,
    comments: 17,
    views: 295,
  },
  {
    id: 19,
    title: '사무실 재활용 분리수거 강화',
    categoryId: 6, category: '환경',
    problem: '현재 분리수거 참여율 40%에 불과. 환경 목표 달성을 위해 개선 필요.',
    solution: '분리수거 구역 재배치 + 직원 교육 + 우수 부서 마일리지 보상으로 참여율 80% 목표.',
    author: '이환경',
    bizArea: '본사',
    department: '총무팀',
    status: '심사대기',
    submittedAt: '2026-03-10',
    security: 'private',
    likes: 22,
    comments: 5,
    views: 76,
  },
  {
    id: 20,
    title: 'VOC 자동 분류 및 우선순위 시스템',
    categoryId: 2, category: '혁신',
    problem: '고객 민원이 하루 평균 200건 이상으로 수동 분류에 한계. 중요 민원 대응 지연.',
    solution: 'NLP 기반 자동 분류 시스템으로 민원 긴급도 자동 판별 및 담당자 자동 배정.',
    author: '김AI',
    bizArea: '부산지사',
    department: '고객서비스팀',
    status: '심사대기',
    submittedAt: '2026-02-25',
    security: 'public',
    likes: 47,
    comments: 13,
    views: 175,
  },
  {
    id: 21,
    title: '제품 QA 체크리스트 표준화',
    categoryId: 5, category: '품질',
    problem: '부서마다 QA 기준이 달라 제품 품질 불균일. 고객 불만 중 품질 관련 비율 40%.',
    solution: 'ISO 기반 표준 체크리스트 구축 및 디지털화. 체크리스트 준수율 100% 목표.',
    author: '이품질',
    bizArea: '강남지사',
    department: '품질팀',
    status: '완료',
    submittedAt: '2025-10-15',
    security: 'public',
    likes: 28,
    comments: 7,
    views: 98,
    ideaScore: 83,
    mileageScore: 4200,
  },
  {
    id: 22,
    title: '마케팅 SNS 자동화 도구 도입',
    categoryId: 2, category: '혁신',
    problem: 'SNS 운영에 담당자가 주당 20시간 이상 소요. 콘텐츠 일관성도 부족.',
    solution: 'Buffer/Hootsuite 도입으로 게시물 스케줄링 자동화. 업무 시간 60% 단축 예상.',
    author: '윤마케팅',
    bizArea: '부산지사',
    department: '마케팅팀',
    status: '심사대기',
    submittedAt: '2026-03-12',
    security: 'private',
    likes: 44,
    comments: 10,
    views: 160,
  },
  {
    id: 23,
    title: '사무용 소모품 통합 구매 제도',
    categoryId: 1, category: '절감',
    problem: '부서별 개별 구매로 소모품 비용 과다 지출. 연간 소모품 비용 1.2억원 발생.',
    solution: '전사 통합 구매 시스템 도입으로 벌크 구매 할인 적용. 연간 3천만원 절감 예상.',
    author: '박재무',
    bizArea: '본사',
    department: '재무팀',
    status: '심사대기',
    submittedAt: '2026-03-03',
    security: 'public',
    likes: 31,
    comments: 8,
    views: 105,
  },
  {
    id: 24,
    title: '야간 설비 에너지 절감 자동화',
    categoryId: 1, category: '절감',
    problem: '야간 가동 불필요한 설비가 계속 전력 소모. 야간 전력비 월 800만원 낭비.',
    solution: '자동 전원 차단 스케줄러 도입으로 야간 전력 사용량 40% 절감 목표.',
    author: '이생산',
    bizArea: '강남지사',
    department: '생산1부',
    status: '승인',
    submittedAt: '2026-01-25',
    security: 'public',
    likes: 37,
    comments: 11,
    views: 130,
    ideaScore: 80,
    mileageScore: 1500,
  },
  {
    id: 25,
    title: '개발 코드 리뷰 가이드라인 및 CI/CD 파이프라인 고도화',
    categoryId: 2, category: '혁신',
    problem: '배포 시간이 평균 45분으로 개발 생산성 저하. 코드 리뷰 기준이 없어 품질 편차 발생.',
    solution: '코드 리뷰 가이드라인 수립 + GitHub Actions CI/CD 자동화. 배포 시간 5분, 리뷰 효율 50% 향상.',
    author: '박개발',
    bizArea: '본사',
    department: '개발팀',
    status: '실행중',
    submittedAt: '2025-12-20',
    security: 'public',
    likes: 59,
    comments: 18,
    views: 215,
    ideaScore: 91,
    mileageScore: 3500,
  },
  {
    id: 26,
    title: '연차 휴가 자동 승인 시스템 도입',
    categoryId: 4, category: '복지',
    problem: '연차 신청 후 승인까지 평균 3일 소요. 급한 휴가 사용이 어려워 직원 불만이 높음.',
    solution: '특정 조건(잔여 연차 보유, 사전 신청)에서 자동승인 처리. 1일 전 신청도 가능하도록 개선.',
    author: '최인사',
    bizArea: '본사',
    department: '인사팀',
    status: '심사대기',
    submittedAt: '2026-03-18',
    security: 'private',
    likes: 103,
    comments: 27,
    views: 420,
  },
  {
    id: 27,
    title: 'UX 리서치 기반 앱 개선 프로세스 정규화',
    categoryId: 5, category: '품질',
    problem: '사용자 피드백 없이 기능 개발이 진행되어 실사용률이 낮은 기능이 다수 존재.',
    solution: '분기별 UX 리서치 + 사용성 테스트 의무화. 데이터 기반 개선 사이클 구축.',
    author: '김UX',
    bizArea: '본사',
    department: 'UX팀',
    status: '심사대기',
    submittedAt: '2026-03-20',
    security: 'public',
    likes: 41,
    comments: 12,
    views: 148,
  },
  {
    id: 28,
    title: '사내 셔틀버스 노선 확대',
    categoryId: 4, category: '복지',
    problem: '출퇴근 시간대 지하철 혼잡 및 지각 증가. 원거리 직원의 출퇴근 부담이 심각.',
    solution: '오전/오후 추가 노선 2개 신설. 수요 기반 탄력 노선 운영으로 직원 만족도 향상.',
    author: '최교통',
    bizArea: '본사',
    department: '총무팀',
    status: '심사대기',
    submittedAt: '2026-03-10',
    security: 'public',
    likes: 67,
    comments: 14,
    views: 225,
  },
  // 데모 계정(홍길동) 작성 아이디어
  {
    id: 101,
    title: '개발팀 온보딩 프로세스 디지털화',
    categoryId: 2, category: '혁신',
    problem: '신입 개발자 온보딩이 수작업·구두 전달에 의존해 인수인계 품질이 들쭉날쭉함.',
    solution: '노션 기반 온보딩 위키 + 자동 체크리스트 시스템 구축. 평균 온보딩 기간 2주 → 1주로 단축.',
    author: MY_AUTHOR,
    bizArea: '본사',
    department: '개발팀',
    status: '심사대기',
    submittedAt: '2026-03-05',
    security: 'public',
    likes: 34,
    comments: 8,
    views: 120,
  },
  {
    id: 102,
    title: '코드 리뷰 가이드라인 표준화',
    categoryId: 5, category: '품질',
    problem: '팀원마다 코드 리뷰 기준이 달라 리뷰 시간이 길고 피드백 일관성이 부족함.',
    solution: '리뷰 체크리스트 + 자동 린트 룰 세트 제정. PR당 평균 리뷰 시간 40% 단축 목표.',
    author: MY_AUTHOR,
    bizArea: '본사',
    department: '개발팀',
    status: '승인',
    submittedAt: '2026-01-20',
    security: 'public',
    likes: 52,
    comments: 15,
    views: 210,
    ideaScore: 82,
    mileageScore: 20000,
  },
  {
    id: 103,
    title: '사내 기술 블로그 운영 제안',
    categoryId: 2, category: '혁신',
    problem: '개발팀 내 지식이 개인에 집중되고 외부 공유 채널이 없어 채용 브랜딩 취약.',
    solution: '주 1회 기술 포스팅 의무화 + 외부 블로그 플랫폼 연동. 분기별 우수 포스팅 포상.',
    author: MY_AUTHOR,
    bizArea: '본사',
    department: '개발팀',
    status: '심사대기',
    submittedAt: '2026-03-14',
    security: 'public',
    likes: 18,
    comments: 4,
    views: 73,
  },
]