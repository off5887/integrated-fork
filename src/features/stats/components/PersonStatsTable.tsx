...
type SortKey = keyof Omit<PersonStatsRow, 'id' | 'name' | 'department' | 'position'>
...
function StatCell({ col, value }: { col: ColDef; value: number }) {
  ...
  return (
    <Box>
      <Typography variant="caption" sx={{ color: textSecondary, display: 'block', mb: 0.2 }}>
        {col.label}
      </Typography>
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Box>
            ...
            <Typography variant="caption" sx={{ color: textSecondary }}>{row.department}</Typography>
          </Box>
        </Box>
        <Chip
          label={row.position}
          ...
          sx={{
            ...
            bgcolor: 'rgba(99,102,241,0.07)', color: '#6366f1',
...
export default function PersonStatsTable({ data }: { data: PersonStatsRow[] }) {
  ...
  const totals = useMemo(
    () => COLS.reduce((acc, col) => { acc[col.key] = sorted.reduce((s, r) => s + r[col.key], 0); return acc }, {} as Record<SortKey, number>),
    [sorted],
  )
    ...
    return (
      <Box>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
          ...
          <Paper
            elevation={0}
            sx={{ border: `2px solid ${borderColor}`, borderRadius: 2, bgcolor: headerBg, overflow: 'hidden' }}
          >
            <Box sx={{ px: 2, py: 1.2, borderBottom: `1px solid ${borderColor}` }}>
              <Typography variant="body2" fontWeight={800} sx={{ color: textPrimary }}>합계</Typography>
            </Box>
            <Box sx={{ px: 2, py: 1.5, display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 1.5 }}>
              {COLS.map((col) => (
  ...
  return (
    <Box>
        ...
        <Table size="small" sx={{ minWidth: 900 }}>
          <TableHead>
            <TableRow sx={{ bgcolor: headerBg }}>
              <TableCell sx={{ ...sticky, bgcolor: headerBg, borderBottom: `1px solid ${borderColor}`, fontWeight: 700, fontSize: '0.75rem', color: textSecondary, whiteSpace: 'nowrap', minWidth: 100 }}>이름</TableCell>
              <TableCell sx={{ borderBottom: `1px solid ${borderColor}`, fontWeight: 700, fontSize: '0.75rem', color: textSecondary, whiteSpace: 'nowrap', minWidth: 90 }}>부서</TableCell>
              <TableCell sx={{ borderBottom: `1px solid ${borderColor}`, fontWeight: 700, fontSize: '0.75rem', color: textSecondary, whiteSpace: 'nowrap', minWidth: 70 }}>직급</TableCell>
              {COLS.map((col) => (
                <TableCell key={col.key} align="center" sortDirection={sortKey === col.key ? sortDir : false}
                  sx={{ borderBottom: `1px solid ${borderColor}`, fontWeight: 700, fontSize: '0.75rem', color: textSecondary, whiteSpace: 'nowrap', minWidth: 90 }}>
          ...
          <TableBody>
              ...
              <TableRow key={row.id} sx={{ bgcolor: idx % 2 === 0 ? 'transparent' : rowBg, '&:hover': { bgcolor: rowHoverBg }, '&:last-child td': { borderBottom: 0 } }}>
                <TableCell sx={{ ...sticky, bgcolor: idx % 2 === 0 ? cardBg : rowBg, borderBottom: `1px solid ${borderColor}`, py: 1.5 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.8 }}>
                    <Box sx={{ width: 26, height: 26, borderRadius: '50%', bgcolor: 'rgba(99,102,241,0.1)', color: '#6366f1', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.7rem', fontWeight: 800, flexShrink: 0 }}>
                      {row.name.slice(0, 1)}
                    </Box>
                    <Typography variant="body2" fontWeight={700} sx={{ color: textPrimary, whiteSpace: 'nowrap' }}>{row.name}</Typography>
                  </Box>
                </TableCell>
                <TableCell sx={{ borderBottom: `1px solid ${borderColor}`, py: 1.5 }}>
                  <Typography variant="body2" sx={{ color: textSecondary, whiteSpace: 'nowrap' }}>{row.department}</Typography>
                </TableCell>
                <TableCell sx={{ borderBottom: `1px solid ${borderColor}`, py: 1.5 }}>
                  <Chip label={row.position} size="small" sx={{ height: 20, fontSize: '0.7rem', fontWeight: 600, bgcolor: 'rgba(99,102,241,0.07)', color: '#6366f1', border: '1px solid rgba(99,102,241,0.18)' }} />
                </TableCell>
                {COLS.map((col) => (
            ...
            <TableRow sx={{ bgcolor: headerBg }}>
              <TableCell colSpan={3} sx={{ ...sticky, bgcolor: headerBg, borderTop: `2px solid ${borderColor}`, py: 1.5 }}>
                <Typography variant="body2" fontWeight={800} sx={{ color: textPrimary }}>합계</Typography>
              </TableCell>
              {COLS.map((col) => (
                <TableCell key={col.key} align="center" sx={{ borderTop: `2px solid ${borderColor}`, py: 1.5 }}>
                  <Typography variant="body2" fontWeight={800} sx={{ color: col.color }}>
                    {col.key === 'expectedAmount' ? totals[col.key].toLocaleString() : totals[col.key]}
                    <Typography component="span" variant="caption" sx={{ color: textSecondary, ml: 0.3 }}>{col.unit}</Typography>
                  </Typography>
                </TableCell>
              ))}
      ...
      <Typography variant="caption" sx={{ color: textSecondary, mt: 1, display: 'block' }}>
        * 컬럼 헤더를 클릭하면 정렬됩니다. 가로로 스크롤하여 전체 통계를 확인하세요.
      </Typography>
    </Box>
  )
}
